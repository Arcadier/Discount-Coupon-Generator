(function ()
{
  ...
  document.addEventListener("DOMContentLoaded", function ()
  {
    ...
    var promocode,
      ...
      couponId;
    var currentSubtotal;
    var mpCurrencycode = $("#currencyWithSymbol").val();
    ...
    function appendCouponDelivery2()
    {
      ...
      var promodiv =
        '<div class="promocode-update" id="promodiv" style="margin-top:10px"><input name="coupon-code" placeholder="PROMOCODE" class="pr-text" maxlength="10" type="text" id="promocode"><button type="button" class="apply-promo-btn" id="applybutton2">Apply</button></div>';
    ...
    function discount_checkout()
    {
      waitForElement("#couponhidden", function ()
      {
        ...
        var promo =
          '<div class="checkout-totline-left" id="coupon_ordetails">tash</div> <div class="checkout-itm-tprice" id="couponvalue">13123213</div> <div class="clearfix"></div>';
        last.append(promo);
        $("#coupon_ordetails").text(couponname);
        $("#couponvalue").text(
          "-" + mpCurrencycode + formatter.format(coupondiscount)
        );
      });
    }
        ...
        if (document.body.className.includes("delievry-settings")) {
          $(".sel_del_method").on("change", function ()
          {
            if ($("#promocode").val() == "") {
              ...
              totalwithDelivery =
                parseFloat(totalwithcoupon) + parseFloat(deliveryCharge);
              console.log("totalwithdel " + totalwithDelivery);
              $("#currencySym").text("-" + mpCurrencycode);
              //total - coupon discount
              $("#price_amt").text(couponvalue);
              //Total
              $(".total_area .total-cost").text(
                formatter.format(totalwithDelivery)
              );
            }
          });
        }
